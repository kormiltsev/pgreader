...
type Settingos struct {
	...
	// TableName   string    `env:"-"`
	// ColimnsName []string  `env:"-"`
	// Limit       int       `env:"-"`
	DB *pgx.Conn `env:"-"`
}

// var Config = Settingos{
// 	DBlink: "",
// TableName:   "urls",
// ColimnsName: make([]string, 0),
// Limit:       100,
// }
...
func main() {
	...
	// to use this outside:
	// PrintPostgressTablesMainInfo(ctx)

	// connect and use during main
	Config, err := NewConnect(ctx, funWithFlags())
	if err != nil {
	...
	// bottom is equal PrintPostgressTablesMainInfo(ctx)
